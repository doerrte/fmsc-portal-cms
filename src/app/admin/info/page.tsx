  ...
  return (
    <div>
      <div style={{ marginBottom: '4rem' }}>
        ...
        <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: '1.2rem', marginBottom: '2rem' }}>Bearbeite Sicherheitsregeln, Gastflieger-Infos und Downloads.</p>
        ...
        <form action={async (fd) => {
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginBottom: '1.5rem' }}>
            ...
            <textarea name="guestRules" defaultValue={info.guestRules} placeholder="Versicherungsnachweis erforderlich&#10;Einweisung durch ein Vereinsmitglied..." style={{ padding: '12px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.2)', background: 'transparent', color: 'white', minHeight: '120px' }} />
          </div>

          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginBottom: '1.5rem' }}>
            <label style={{ fontWeight: 'bold' }}>Hinweis-Kasten (z.B. Wegbeschreibung / Warnung)</label>
            <input name="guestWarning" defaultValue={info.guestWarning} placeholder="Anfahrt: Bitte auf den Wirtschaftswegen extrem langsam fahren" style={{ padding: '12px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.2)', background: 'transparent', color: 'white' }} />
          </div>
      ...
      <div style={{ marginBottom: '4rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '1.5rem' }}>
          ...
          {!editingSafety && (
            <button onClick={() => setEditingSafety({ id: '', title: '', desc: '', icon: 'Shield' })} style={{ background: '#f97316', color: 'white', padding: '10px 20px', borderRadius: '8px', border: 'none', fontWeight: 'bold', cursor: 'pointer' }}>
              + Regel hinzufügen
            </button>
          )}
        </div>
            ...
            <form action={async (fd) => { await saveInfoSafetyRule(fd); window.location.reload(); }} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div style={{ display: 'flex', gap: '1rem' }}>
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  ...
                  <input name="title" defaultValue={editingSafety.title} style={{ padding: '12px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.2)', background: 'transparent', color: 'white' }} required />
                </div>
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  <label style={{ fontWeight: 'bold' }}>Icon Symbol</label>
                  <select name="icon" defaultValue={editingSafety.icon} style={{ padding: '12px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.2)', background: '#0a0c10', color: 'white' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                ...
                <textarea name="desc" defaultValue={editingSafety.desc} style={{ padding: '12px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.2)', background: 'transparent', color: 'white', minHeight: '80px' }} required />
              </div>

              <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem' }}>
                <button type="submit" style={{ background: '#f97316', color: 'white', padding: '10px 20px', borderRadius: '8px', border: 'none', fontWeight: 'bold', cursor: 'pointer' }}>Speichern</button>
                <button type="button" onClick={() => setEditingSafety(null)} style={{ background: 'transparent', border: '1px solid rgba(255,255,255,0.2)', color: 'white', padding: '10px 20px', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer' }}>Abbrechen</button>
              </div>
            </form>
          </div>
        ) : (
                ...
                <div key={r.id} style={{ background: 'rgba(255,255,255,0.05)', padding: '1.5rem', borderRadius: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'flex-start' }}>
                    ...
                    <div>
                      ...
                      <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.9rem' }}>{r.desc}</p>
                    </div>
                  </div>
      ...
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '1.5rem' }}>
          ...
          {!editingDoc && (
            <button onClick={() => setEditingDoc({ id: '', title: '', url: '', sizeInfo: '' })} style={{ background: '#f97316', color: 'white', padding: '10px 20px', borderRadius: '8px', border: 'none', fontWeight: 'bold', cursor: 'pointer' }}>
              + Dokument hochladen
            </button>
          )}
        </div>
          ...
          <div style={{ background: 'rgba(255,255,255,0.05)', padding: '2rem', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.1)' }}>
            <form action={async (fd) => { await saveInfoDoc(fd); window.location.reload(); }} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginTop: '1rem' }}>
                ...
                {editingDoc.url && <p style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.5)' }}>Aktuelle Datei: {editingDoc.url} ({editingDoc.sizeInfo})</p>}
              ...
              <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem' }}>
                <button type="submit" style={{ background: '#f97316', color: 'white', padding: '10px 20px', borderRadius: '8px', border: 'none', fontWeight: 'bold', cursor: 'pointer' }}>Speichern</button>
                <button type="button" onClick={() => setEditingDoc(null)} style={{ background: 'transparent', border: '1px solid rgba(255,255,255,0.2)', color: 'white', padding: '10px 20px', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer' }}>Abbrechen</button>
              </div>
            </form>
          </div>
        ) : (
              ...
              <div key={doc.id} style={{ background: 'rgba(255,255,255,0.05)', padding: '1.5rem', borderRadius: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                  ...
                  <div>
                    ...
                    <span style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.5)' }}>PDF | {doc.sizeInfo}</span>
                  </div>
                </div>
                <div style={{ display: 'flex', gap: '0.5rem' }}>
